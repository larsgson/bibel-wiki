...
import { getTabExercises, getOverflowExercises } from "./ExerciseRegistry";
...
function ExerciseTabBar({ activeExerciseId, onSelectExercise, layoutTheme }) {
  const { t } = useTranslation();
  const [moreOpen, setMoreOpen] = useState(false);
  const moreRef = useRef(null);

  const tabExercises = getTabExercises();
  const overflowExercises = getOverflowExercises();
  ...
  return (
    <nav className={`exercise-tab-bar${layoutTheme ? ` theme-${layoutTheme}` : ""}`}>
